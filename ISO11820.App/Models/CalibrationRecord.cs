...
/// <summary>
/// 设备校准历史记录（对应 CalibrationRecords 表）
/// </summary>
public class CalibrationRecord
{
    public string Id { get; set; } = Guid.NewGuid().ToString(); // GUID 主键
    public string CalibrationDate { get; set; } = string.Empty; // ISO 8601 字符串
    public string CalibrationType { get; set; } = string.Empty; // "Surface" 或 "Center"
    public int ApparatusId { get; set; }
    public string Operator { get; set; } = string.Empty;
    public string TemperatureData { get; set; } = string.Empty; // JSON 字符串
